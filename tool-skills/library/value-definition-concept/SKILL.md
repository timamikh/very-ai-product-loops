...
stage: the **base** moats only, each tested against the post-AI premise, with the killed candidates
kept. Fills `{#value-defensibility}` at Step 1; the Step-3 revisit (`value-definition-strategy`)
adds derivative moats and the moat trajectory once customers/scale exist.
...
- Any time a "why us / why can't this be copied" question is on the table before a strategy exists.

## The taxonomy — base values only, at this step
...
3. **Defer derivatives with one line.** Leave a single line in the section — "derivative moats
   deferred to Step 3 (`value-definition-strategy`); they need a customer/scale" — rather than
   silently omitting them, so a reader knows they were considered, not forgotten. Do not leave an
   empty table.
...
- **Derivative at concept stage.** Claiming network effects / lock-in / scale before there is
  an audience or a customer — that claim belongs to `value-definition-strategy`, once there is
  something to derive it from.
- **Aspiration stated as fact.** A moat you *hope* to build tagged `[validated]` instead of
  `[assumption]`.
- **Hiding the kills.** Dropping the candidates the rebuild test rejected — the next pass
  re-proposes them.
...
Projects `{#value-defensibility}` via [`template-fragment.md`](template-fragment.md) from its
worklog; inputs gathered via [`questions.yaml`](questions.yaml); each unproven moat seeds `H-…`.
The Step-3 revisit is `value-definition-strategy` — derivatives, trajectory, and the contribution
to `{#how-to-win}`/`{#bets}`. Deeper notes:
