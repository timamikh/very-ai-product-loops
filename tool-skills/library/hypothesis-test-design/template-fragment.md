...
_The threshold and `M-…` link come from Step 4 (`{#global-hypotheses}`); this section adds the
smallest sufficient test that can reach that bar. A row with no decision rule set in advance isn't a
test._
