...
1. **The step's own sections are all worked** and the gate's derived items (`#to-clarify`,
   `#hypotheses`, an optional section) are still open — the common case, immediately before
   `theses`.
...
- **It belongs to the step** → an open item in `#to-clarify`, or a register entry (a tension between
  two sections is often a risk; a bet nobody tested is a hypothesis). Name which of the two kinds of
  open item it is: *the human must choose*, or *nobody knows and someone must go and find out*.
...
- **Re-working content under cover of synthesis.** Rewriting a section's substance because the whole
  ...
  `verify`; this pass names the finding and stops.
...
- **Open items** in `#to-clarify`, each marked as *the human chooses* or *nobody knows yet*, and
  register entries for the tensions and bets the whole surfaced.
