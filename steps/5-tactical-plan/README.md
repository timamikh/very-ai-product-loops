...
method_basis: "OKR-style goals per direction · targets = metric nodes (growth) or DoD (technical & back-office) · guardrails / red lines (steering-committee reconciliation) · resource survey · experiment design · prioritization (RICE/ICE)"
status: draft
version: 0.1.0
updated: 2026-07-16
---
...
| `goal-targets` | What each goal maps to: **growth → metric node (`M-…`); technical & back-office → a Definition of Done** | `metric-tree` |
...
- **Metric tree** — growth goals select nodes to move (`M-…`); guardrails are protected `M-…` nodes.
...
- [ ] growth goals map to metric nodes; technical & back-office goals map to a DoD → `tactical-plan#goal-targets` → metric register
